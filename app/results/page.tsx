...
import { Leaf, ArrowLeft, CheckCircle2, Loader2, AlertCircle } from 'lucide-react'
import {
  checkJobStatus,
  parseClimateResult,
  getAQIColor,
  ...
  const loadResults = async () => {
    try {
      ...
      if (status.status !== 'completed') {
        setError(`Job is not completed yet. Current status: ${status.status}`)
        return
      }
      ...
      const climateData = parseClimateResult(status.result)
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-xl font-bold">Monitoring Results</h1>
                <p className="text-xs text-muted-foreground">Job ID: {jobId.slice(0, 8)}...</p>
              </div>
            </div>
          </div>
        </div>
